export const tenantData = {
  id: 'PRJ-25-SIP002',
  companyName: 'PT. Maju Mundur Bersama',
  pic: 'Budianta',
  slug: 'pt-consultan-pajak-maju',
  picEmail: 'Budi@gmail.com',
  status: 'Active',
  createdAt: '12/01/2024, 17:00',
  plan: 'Pro',
  lastActivity: '12/01/2024, 17:00',
};

export const statsData = {
  activeProjects: {
    current: 5,
    total: 8,
  },
  totalUsers: {
    current: 83,
    total: 100,
    label: 'Across all tenants',
  },
  storageUsage: {
    percentage: 85,
    used: '85,5 GB',
    total: '100 GB',
  },
  uptimeSLA: {
    percentage: 100,
  },
};

export const metricsData = [
  {
    icon: '📁',
    label: 'Projects',
    value: '15',
    total: '28',
    bgColor: 'bg-blue-50',
    iconColor: 'text-blue-600',
  },
  {
    icon: '👥',
    label: 'Active Users',
    value: '10',
    total: '25',
    bgColor: 'bg-green-50',
    iconColor: 'text-green-600',
  },
];

export const usageTrendData = [
  {
    value: '2.3k',
    label: 'API Calls/Day',
    change: '+12%',
    color: 'text-blue-600',
    changePositive: true,
  },
  {
    value: '156',
    label: 'Documents/Day',
    change: '+8%',
    color: 'text-green-600',
    changePositive: true,
  },
  {
    value: '890',
    label: 'Logins/Day',
    change: '-3%',
    color: 'text-purple-600',
    changePositive: false,
  },
  {
    value: '1.2GB',
    label: 'Storage/Day',
    change: '+15%',
    color: 'text-orange-600',
    changePositive: true,
  },
];

export const usersData = [
  {
    id: '1',
    name: 'User 1',
    email: 'User@example.com',
    status: 'Active',
    role: 'Executive',
    lastLogin: '12 September 2025 13:20',
  },
  {
    id: '2',
    name: 'User 2',
    email: 'User@example.com',
    status: 'Inactive',
    role: 'PMO',
    lastLogin: '12 September 2025 13:20',
  },
  {
    id: '3',
    name: 'User 3',
    email: 'User@example.com',
    status: 'Active',
    role: 'Anggota Tim',
    lastLogin: '12 September 2025 13:20',
  },
  {
    id: '4',
    name: 'User 4',
    email: 'User@example.com',
    status: 'Inactive',
    role: 'Ketua Tim',
    lastLogin: '12 September 2025 13:20',
  },
  {
    id: '5',
    name: 'User 5',
    email: 'User@example.com',
    status: 'Active',
    role: 'Executive',
    lastLogin: '12 September 2025 13:20',
  },
  {
    id: '6',
    name: 'User 6',
    email: 'User@example.com',
    status: 'Inactive',
    role: 'Anggota Tim',
    lastLogin: '12 September 2025 13:20',
  },
  {
    id: '7',
    name: 'User 7',
    email: 'User@example.com',
    status: 'Active',
    role: 'Anggota Tim',
    lastLogin: '12 September 2025 13:20',
  },
  {
    id: '8',
    name: 'User 8',
    email: 'User@example.com',
    status: 'Active',
    role: 'Anggota Tim',
    lastLogin: '12 September 2025 13:20',
  },
];

export const projectsData = [
  {
    id: 'PRJ-25-SIP002',
    companyName: 'PT. SUKSES INDO PRAMATA',
    team: 'Budi Dharma',
    milestones: [
      { name: 'Form 1.0', status: 'Done' },
      { name: 'KK 1.0', status: 'Done' },
      { name: 'KK 2.0', status: 'Done' },
      { name: 'KK 3.0', status: 'In Progress' },
      { name: 'KK 4.0', status: 'Not started' },
      { name: 'KK 5.0', status: 'Not started' },
    ],
    nextMilestone: {
      name: 'KK 3.0 Completion',
      date: '2025-09-20',
    },
    progress: 50,
  },
  {
    id: 'PRJ-25-SIP003',
    companyName: 'CV. BERKAH SEJAHTERA',
    team: 'Andi Pratama',
    milestones: [
      { name: 'Form 1.0', status: 'Done' },
      { name: 'KK 1.0', status: 'In Progress' },
      { name: 'KK 2.0', status: 'Not started' },
      { name: 'KK 3.0', status: 'Not started' },
      { name: 'KK 4.0', status: 'Not started' },
      { name: 'KK 5.0', status: 'Not started' },
    ],
    nextMilestone: {
      name: 'KK 1.0 Completion',
      date: '2025-10-05',
    },
    progress: 20,
  },
  {
    id: 'PRJ-25-SIP004',
    companyName: 'PT. GLOBAL TEKNOLOGI',
    team: 'Siti Rahayu',
    milestones: [
      { name: 'Form 1.0', status: 'In Progress' },
      { name: 'KK 1.0', status: 'Not started' },
      { name: 'KK 2.0', status: 'Not started' },
      { name: 'KK 3.0', status: 'Not started' },
      { name: 'KK 4.0', status: 'Not started' },
      { name: 'KK 5.0', status: 'Not started' },
    ],
    nextMilestone: {
      name: 'Form 1.0 Completion',
      date: '2025-10-15',
    },
    progress: 8,
  },
];

export const systemsBillingData = {
  currentPlan: 'Pro',
  billingCycle: 'Monthly',
  amount: 'Rp 2.500.000',
  nextBillingDate: '15/4/2024',
  paymentMethod: 'Bank Transfer',
};

export const systemsAuditTrailData = [
  {
    action: 'TENANT_CREATED',
    performedBy: 'Platform Admin',
    timestamp: '15/1/2024, 17.00.00',
    severity: 'medium',
  },
  {
    action: 'PLAN_CHANGED',
    performedBy: 'Platform Admin',
    timestamp: '15/2/2024, 21.30.00',
    severity: 'medium',
  },
];

export const systemsUsageData = {
  users: {
    active: 10,
    limit: 25,
    percentage: 40,
  },
  apiCalls: {
    thisMonth: 8540,
    limit: 50000,
    percentage: 17.08,
  },
  storage: {
    used: '45.5 GB',
    limit: '100 GB',
    percentage: 45.5,
  },
};

export const systemsBrandingData = {
  companyName: 'PT Konsultan Pajak Maju',
  tagline: 'Solusi Pajak Terpercaya',
  primaryColor: '#2563eb',
  logo: null,
};

export const systemsIntegrationsData = [
  {
    name: 'Accurate Online',
    category: 'ERP',
    description: 'Sistem akuntansi terintegrasi untuk sinkronisasi data keuangan',
    lastSync: '10/3/2024, 13.00.00',
    status: 'connected',
  },
  {
    name: 'ZAHIR Accounting',
    category: 'Accounting',
    description: 'Software akuntansi untuk manajemen laporan keuangan',
    lastSync: '10/3/2024, 11.30.00',
    status: 'connected',
  },
  {
    name: 'Google Drive',
    category: 'Storage',
    description: 'Cloud storage untuk backup dokumen',
    lastSync: null,
    status: 'disconnected',
  },
];

export const systemsAvailableIntegrationsData = [
  {
    name: 'SAP Business One',
    category: 'ERP',
    description: 'Enterprise resource planning integration',
  },
  {
    name: 'Oracle NetSuite',
    category: 'ERP',
    description: 'Cloud-based business management suite',
  },
  {
    name: 'Microsoft Dynamics',
    category: 'CRM',
    description: 'Customer relationship management',
  },
  {
    name: 'Slack',
    category: 'Communication',
    description: 'Team collaboration and messaging',
  },
  {
    name: 'Dropbox',
    category: 'Storage',
    description: 'Cloud file storage and sharing',
  },
  {
    name: 'OneDrive',
    category: 'Storage',
    description: 'Microsoft cloud storage solution',
  },
];
