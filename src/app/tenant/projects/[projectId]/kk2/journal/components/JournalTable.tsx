...
export function JournalTable({ data }: JournalTableProps) {
  ...
  return (
    <div className="flex flex-col items-start gap-[50px] self-stretch rounded-[20px]">
      <div className="flex flex-col items-start gap-5 self-stretch">
        ...
        <div className="flex items-start justify-between self-stretch">
          ...
          <Link href={`/tenant/projects/${projectId}/kk2/journal/add`}>
            <Button className="h-12 gap-1 rounded-[10px] bg-[#332687] hover:bg-[#332687]/90">
              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
        ...
        <div className="flex items-center gap-5 self-stretch rounded-[10px]">
          <Select value={pageSize} onValueChange={setPageSize}>
            <SelectTrigger className="h-[54px] w-auto gap-1 rounded-[10px] border border-[#D9D9D9]">
              <SelectValue />
          ...
          <div className="relative flex h-[54px] flex-1 items-center gap-[15px] rounded-[10px] border border-[#D9D9D9] px-4">
            ...
            <Input
              ...
              className="h-full border-0 p-0 text-base font-normal leading-5 tracking-[-0.32px] text-[#8F9BBA] placeholder:text-[#8F9BBA] focus-visible:ring-0"
            />
          </div>

          <div className="flex items-center gap-2.5">
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              <SelectTrigger className="h-[54px] gap-1 rounded-[10px] border border-[#D9D9D9]">
        ...
        {data.map((entry) => (
          ...
          >
            <div className="flex w-[120px] flex-col items-start gap-2.5">
              <span className="text-sm font-medium leading-[14px] text-[#404040]">
                {entry.date}
              </span>
            </div>
            <div className="flex w-[150px] items-center gap-2.5">
              <span className="w-[120px] flex-shrink-0 text-sm font-medium leading-[14px] text-[#404040]">
                {entry.number}
              </span>
            </div>
            <div className="flex w-[120px] flex-col items-start gap-2.5">
            ...
            <div className="flex h-12 w-[150px] items-center gap-2.5">
              <Link href={`/tenant/projects/${projectId}/kk2/journal/${entry.id}`}>
                <Button variant="ghost" size="icon" className="h-[25px] w-[25px]">
