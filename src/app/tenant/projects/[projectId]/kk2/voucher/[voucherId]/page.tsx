...
export default function VoucherDetailPage() {
	...
	return (
		<div className="flex min-h-screen w-full flex-col gap-[35px] bg-card p-5">
			{/* Header */}
			<div className="flex items-start justify-between border-b pb-3.5">
				<div className="flex flex-col gap-1.5">
					<h2 className="font-inter text-sm font-semibold leading-[21px] ">
						Detail Voucher
					</h2>
					<p className="font-inter text-xs leading-[17.5px] text-[#64748B]">
			...
			<div className="flex flex-col gap-3.5">
				...
				<Card className="rounded-[10.5px] border-[0.8px] border-[rgba(0,0,0,0.15)]">
					<CardHeader className="flex flex-row items-center justify-between space-y-0 p-[22px] pb-0">
						<div className="flex items-center gap-2.5">
							...
							<span className="font-inter text-base leading-[24.5px] ">
					...
					<CardContent className="flex flex-col gap-3.5 p-[22px] pt-[26px]">
						...
						<div className="grid grid-cols-1 gap-3.5 md:grid-cols-2">
							<div className="flex flex-col gap-1.5">
								...
								<p className="font-inter text-sm font-medium leading-[21px] ">
						...
						<div className="flex flex-col gap-1.5">
							...
							<p className="font-inter text-xs leading-[17.5px] ">
				...
				<Card className="rounded-[10.5px] border-[0.8px] border-[rgba(0,0,0,0.15)]">
					<CardHeader className="p-[21px] pb-0">
						<CardTitle className="font-inter text-base font-medium leading-[24.5px] tracking-[-0.394px] ">
							Baris Jurnal
						</CardTitle>
					</CardHeader>
						...
						<Table>
							<TableHeader>
								<TableRow className="border-b-[0.8px] border-[rgba(0,0,0,0.15)] bg-[rgba(248,250,252,0.5)]">
									<TableHead className="h-[35px] px-[7px] py-2 font-inter text-xs font-medium leading-[17.5px] ">
										No. Akun
									</TableHead>
									<TableHead className="h-[35px] px-[7px] py-2 font-inter text-xs font-medium leading-[17.5px] ">
										Nama Akun
									</TableHead>
									<TableHead className="h-[35px] px-[7px] py-2 text-right font-inter text-xs font-medium leading-[17.5px] ">
										Debit
									</TableHead>
									<TableHead className="h-[35px] px-[7px] py-2 text-right font-inter text-xs font-medium leading-[17.5px] ">
										Kredit
							...
							<TableBody>
								{voucherDetailData.journalLines.map((line, index) => (
									<TableRow key={index} className="border-0">
										<TableCell className="h-[42px] px-[7px] py-3 font-inter text-xs font-medium leading-[17.5px] ">
											{line.accountNo}
										</TableCell>
										<TableCell className="h-[42px] px-[7px] py-3 font-inter text-xs leading-[17.5px] ">
											{line.accountName}
										</TableCell>
										<TableCell className="h-[42px] px-[7px] py-3 text-right font-inter text-xs leading-[17.5px] ">
											{line.debit}
										</TableCell>
										<TableCell className="h-[42px] px-[7px] py-3 text-right font-inter text-xs leading-[17.5px] ">
											{line.credit}
										...
										<TableCell className="h-[42px] px-[7px] py-2">
											{line.refKK1 && (
												<Button
													variant="ghost"
													size="sm"
													className="h-7 gap-1 rounded-[5.25px] px-2 font-inter text-[10.5px] font-medium leading-[14px] "
												>
										...
										<TableCell className="h-[42px] px-[7px] py-3 font-inter text-xs leading-[17.5px] text-[#64748B]">
											{line.notes}
										</TableCell>
									</TableRow>
								))}
						...
						{voucherDetailData.attachments.map((attachment) => (
							...
							>
								<div className="flex items-center gap-2">
									...
									<span className="font-inter text-xs font-medium leading-[17.5px] ">
										{attachment.name}
									</span>
								</div>
								<Button
							...
							{voucherDetailData.auditTrail.map((trail, index) => (
								<div key={index} className="relative flex gap-4 pb-6">
									<div className="relative">
										<div className="h-[7px] w-[7px] rounded-full " />
									</div>
									<div className="flex flex-1 flex-col gap-0">
										<div className="flex items-end gap-1.5">
											...
											<span className="font-inter text-xs leading-[17.5px] ">
												oleh {trail.user}
											</span>
										</div>
										<p className="font-inter text-xs leading-[17.5px] text-[#64748B]">
										...
										<p className="font-inter text-[10.5px] leading-[14px] text-[#64748B]">
											{trail.timestamp}
										</p>
									</div>
								</div>
