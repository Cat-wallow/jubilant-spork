...
const recentVouchers = [
	...
	},
	{
		...
		status: "Ready",
		amount: "Rp 5.500.000",
		statusColor: "bg-gray-100 bg-muted",
	},
	{
		id: 4,
		type: "Draft",
					...
					<Card className="flex flex-1 flex-col gap-2.5 rounded-[14px] border-[0.8px] border-[#E2E8F0] shadow-[0_1px_3px_0_rgba(0,0,0,0.10),0_1px_2px_-1px_rgba(0,0,0,0.10)]">
						<CardHeader className="flex items-center border-b-[0.8px] border-[#E2E8F0] p-6">
							<div className="flex h-8 items-center gap-2">
								...
								<div className="text-base leading-4 ">Recent Vouchers</div>
							</div>
						</CardHeader>
						<CardContent className="flex flex-col gap-3 p-6">
							{recentVouchers.map((voucher) => (
								<div
									key={voucher.id}
									className="flex h-[68px] items-center justify-between rounded-[10px] p-3"
								>
									...
									<div className="flex flex-col items-end gap-1">
										<Badge
											className={`rounded-lg border-0 ${voucher.statusColor}`}
										>
											{voucher.status}
										</Badge>
										<div className="text-xs leading-4 text-[#62748E]">
											{voucher.amount}
										</div>
									</div>
								</div>
					...
					<Card className="flex flex-1 flex-col gap-2.5 rounded-[14px] border-[0.8px] border-[#E2E8F0] shadow-[0_1px_3px_0_rgba(0,0,0,0.10),0_1px_2px_-1px_rgba(0,0,0,0.10)]">
						<CardHeader className="flex items-center border-b-[0.8px] border-[#E2E8F0] p-6">
							<div className="flex h-8 items-center gap-2">
								...
								<div className="text-base leading-4 ">Top Account Balances</div>
							</div>
						</CardHeader>
						<CardContent className="flex flex-col gap-3 p-6">
							{topAccounts.map((account, idx) => (
								<div
									key={idx}
									className="flex h-16 items-center justify-between rounded-[10px]"
								>
									<div className="flex items-center gap-3">
