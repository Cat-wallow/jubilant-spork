...
function StatementCard({
  ...
  return (
    <div className={`flex h-[194px] flex-1 flex-col gap-4 rounded-[10px] border-[0.8px] ${borderColor} p-4`}>
      <div className="flex h-10 items-center gap-3">
        <div className="flex h-8 w-8 flex-col items-start rounded-[10px]">{icon}</div>
        <div className="flex flex-col">
          <div className={`text-base font-bold leading-6 ${titleColor}`}>{title}</div>
          <div className={`text-xs font-normal leading-4 ${periodColor}`}>{period}</div>
        </div>
      </div>
      <div className="flex flex-col gap-3">
        {items.map((item, idx) => (
          <div key={idx} className="flex h-6 items-center justify-between">
            <div className="text-sm font-normal leading-5 text-[#45556C]">{item.label}</div>
            <div className={`font-mono text-base font-bold leading-6 ${item.color}`}>{item.value}</div>
          </div>
        ))}
        <div className="flex h-[33px] flex-col border-t-[0.8px] border-[rgba(0,0,0,0.10)] pt-2">
          <div className="flex h-6 items-center justify-between">
            <div className="text-base font-bold leading-6 text-[#0F172B]">{total.label}</div>
            <div className={`font-mono text-base font-bold leading-6 ${total.color}`}>{total.value}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <Card className="flex flex-col gap-2.5 self-stretch rounded-[14px] border-[0.8px] border-[#E2E8F0] shadow-[0_1px_3px_0_rgba(0,0,0,0.10),0_1px_2px_-1px_rgba(0,0,0,0.10)]">
      <CardHeader className="flex items-center border-b-[0.8px] border-[#E2E8F0] p-6">
        <div className="flex h-9 items-center gap-2">
          ...
          <div className="text-base font-normal leading-4 text-[#0F172B]">
      ...
      <CardContent className="flex items-center gap-[30px] p-6">
        <StatementCard
          ...
          total={{ label: 'Net Income', value: 'Rp 300M', color: 'text-[#007A55]' }}
        />
        <StatementCard
          title="Balance Sheet"
          period="As of Today"
          ...
          items={[
            { label: 'Total Assets', value: 'Rp 15.2B', color: 'text-[#1447E6]' },
            { label: 'Total Liabilities', value: 'Rp 8.1B', color: 'text-[#F54900]' },
          ]}
        ...
        <StatementCard
          ...
          total={{ label: 'Net Change', value: '+Rp 270M', color: 'text-[#096]' }}
        />
      </CardContent>
    </Card>
  );
