        ...
        <div className="flex h-[427px] flex-col justify-center p-4">
          <div className="relative h-full w-full">
            ...
            <div className="absolute inset-0 flex flex-col justify-between">
              {[0, 1, 2, 3, 4].map((i) => (
                <div key={i} className="border-t border-dashed border-[#CCCCCC]" />
              ))}
            </div>
              ...
              <polyline
                points={data
                  .map((item, index) => {
                    ...
                    const y = 100 - ((item.amount - minAmount) / (maxAmount - minAmount)) * 80;
                    return `${x}%,${y}%`;
                  })
                  .join(' ')}
                fill="none"
            ...
            <div className="absolute bottom-0 flex w-full justify-around">
              {data.map((item, index) => (
                <p key={index} className="font-inter text-xs text-[#666666]">
                  {item.month}
                </p>
              ))}
            </div>
