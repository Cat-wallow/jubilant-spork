	...
	return (
		<div className="grid grid-cols-1 gap-[14px] md:grid-cols-2 lg:grid-cols-4">
			{stats.map((stat) => {
				...
				const BadgeIcon = stat.badgeIcon ? iconMap[stat.badgeIcon] : null;
					...
					>
						...
						<div className="flex items-start justify-between">
							...
							{stat.badge && (
								...
								>
									{BadgeIcon && <BadgeIcon className="h-[10.5px] w-[10.5px]" />}
									{stat.badge}
								</div>
							)}
						</div>
						...
						<div className="flex flex-col gap-[3.5px]">
							...
							{stat.progress !== undefined ? (
								<Progress
									value={stat.progress}
									className="h-[5.25px] rounded-full bg-[#D0FAE5]"
									indicatorClassName="rounded-full bg-[#096]"
								/>
							) : (
								stat.description && (
									<p
