...
export default function KertasKerjaPerpajakan({ taxTypeOptions }: KertasKerjaPerpajakanProps) {
  ...
  // transactionTaxes is now an object { tax_deposit: number, ... }
  ...
  const totalTaxNominal = useMemo(() => {
    const fields = ['tax_deposit', 'ppn', 'pph_21', 'pph_23', 'pph_4_2', 'pph_credit', 'other_pph'];
    return fields.reduce((sum, field) => sum + (Number(transactionTaxes[field]) || 0), 0);
  }, [transactionTaxes]);

  // Determine if any tax input is filled for conditional validation
  const isAnyTaxInputFilled = useMemo(() => {
    const fields = ['tax_deposit', 'ppn', 'pph_21', 'pph_23', 'pph_4_2', 'pph_credit', 'other_pph'];
    return fields.some(field => transactionTaxes[field] !== undefined && transactionTaxes[field] !== null && transactionTaxes[field] !== 0 && transactionTaxes[field] !== "");
  }, [transactionTaxes]);
  ...
  const handleFilesAdded = (files: File[] | null) => {
    if (files && files.length > 0) {
      ...
      setValue("tax_proof_files", [...taxProofFiles, ...newFiles], { shouldValidate: true });
    }
  };

  const handleRemoveFile = (id: string) => {
    setValue("tax_proof_files", taxProofFiles.filter((file: any) => file.id !== id), { shouldValidate: true });
  };
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="flex gap-5 items-center">
            <div className="flex flex-1 flex-col gap-2">
              ...
              <Input type="number" variant="idr" placeholder="0" {...register("transaction_taxes.tax_deposit", { valueAsNumber: true })} />
            </div>
        </div>
        <div className="flex gap-5 items-center">
            <div className="flex flex-1 flex-col gap-2">
              <Label>PPN (KK-1.7.2)</Label>
              <Input type="number" variant="idr" placeholder="0" {...register("transaction_taxes.ppn", { valueAsNumber: true })} />
            </div>
        </div>
        <div className="flex gap-5 items-center">
            <div className="flex flex-1 flex-col gap-2">
              <Label>PPh Pasal 21 (KK-1.7.3)</Label>
              <Input type="number" variant="idr" placeholder="0" {...register("transaction_taxes.pph_21", { valueAsNumber: true })} />
            </div>
        </div>
        <div className="flex gap-5 items-center">
            <div className="flex flex-1 flex-col gap-2">
              <Label>PPh Pasal 23 (KK-1.7.3)</Label>
              <Input type="number" variant="idr" placeholder="0" {...register("transaction_taxes.pph_23", { valueAsNumber: true })} />
            </div>
        </div>
        <div className="flex gap-5 items-center">
            <div className="flex flex-1 flex-col gap-2">
              <Label>PPh Pasal 4(2) (KK-1.7.3)</Label>
              <Input type="number" variant="idr" placeholder="0" {...register("transaction_taxes.pph_4_2", { valueAsNumber: true })} />
            </div>
        </div>
        <div className="flex gap-5 items-center">
            <div className="flex flex-1 flex-col gap-2">
              <Label>Piutang/Kredit PPh dipot/put pihak lain (KK-1.7.6)</Label>
              <Input type="number" variant="idr" placeholder="0" {...register("transaction_taxes.pph_credit", { valueAsNumber: true })} />
            </div>
        </div>
        <div className="flex gap-5 items-center">
            <div className="flex flex-1 flex-col gap-2">
              <Label>P2Ph Lainnya (KK-1.7.7)</Label>
              <Input type="number" variant="idr" placeholder="0" {...register("transaction_taxes.other_pph", { valueAsNumber: true })} />
            </div>
        </div>
      </div>
        ...
        {taxProofFiles.length > 0 && (
          <div className="flex flex-col gap-2 mb-2">
            {taxProofFiles.map((file: any, fileIndex: number) => (
              <div key={file.id || fileIndex} className="flex items-center justify-between rounded-md border p-2 bg-white">
                <div className="flex items-center gap-2">
                    <FileText className="h-4 w-4 text-blue-500" />
                    <span className="text-sm truncate max-w-[200px]">{file.file_name}</span>
                </div>
                <Button type="button" variant="ghost" size="icon" onClick={() => handleRemoveFile(file.id)} className="h-8 w-8">
