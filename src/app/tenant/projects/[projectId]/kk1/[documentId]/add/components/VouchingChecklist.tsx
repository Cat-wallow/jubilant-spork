...
import {
	...
	SelectTrigger,
	SelectValue,
} from "@/components/ui/select"; // For COA selection
...
export default function VouchingChecklist({
	coaOptions,
}: VouchingChecklistProps) {
	...
	return (
		<Card className="rounded-[20px] border  p-5">
			<div className="mb-5 flex items-start justify-between ">
				...
				<Button className="flex h-12 items-center gap-2 rounded-[10px]">
			...
			<div className="grid grid-cols-2 gap-6 mb-5">
					...
					<div className="flex items-center space-x-2">
						<Checkbox
							...
							onCheckedChange={(checked) =>
								setValue("document_warning", checked)
							}
						/>
						<label
				...
				<div className="flex flex-col gap-2.5">
					<Label htmlFor="vouching_notes" className=" text-sm ">
						Vouching Notes
					</Label>
					<Textarea
						id="vouching_notes"
						...
						<div className="flex flex-col gap-3 rounded-[10px] border border-[#FFF085] bg-[#FEFCE8] p-4">
							<div className="flex items-start gap-3">
								...
								<div className="flex flex-col">
									<span className=" text-base text-[#733E0A]">Warning</span>
									<span className=" text-sm text-[#A65F00]">
			...
			<div className="mb-5 flex items-center justify-between">
				<div className="flex items-center gap-2">
					...
					<span className=" text-base ">Simulasi Voucher Yang Akan Dibuat</span>
				</div>
			...
			<div className="mb-5 flex items-center justify-between rounded-[10px] bg-[rgba(236,236,240,0.30)] p-4">
				<div className="flex flex-col">
					...
					<span className="text-base ">RJV</span>
				</div>
				<div className="flex flex-col">
					<span className="text-xs text-muted-foreground">Voucher No</span>
					<span className="text-base ">RJV-2025-184</span>
				</div>
				<div className="flex flex-col">
					<span className="text-xs text-muted-foreground">Date</span>
					<span className="text-base ">2024-03-15</span>
				</div>
				<div className="flex flex-col">
			...
			<div className="mb-4 rounded-[10px] bg-[#EFF6FF] p-3">
				...
				<p className="text-sm ">Pengakuan sales kepada PT. ABC Corp sesuai</p>
			</div>
					...
					<TableBody>
						{journalLines?.map((line, index) => (
							...
							>
								...
								<TableCell>
									<Select
										onValueChange={(value) => handleCoaChange(value, line.id)}
									...
									<Input
										...
										onChange={(e) =>
											setJournalLines(
												journalLines?.map((l) =>
													l.id === line.id
														? { ...l, debit: parseFloat(e.target.value) || 0 }
														: l,
												),
											)
										}
									...
									<Input
										...
										onChange={(e) =>
											setJournalLines(
												journalLines?.map((l) =>
													l.id === line.id
														? { ...l, credit: parseFloat(e.target.value) || 0 }
														: l,
												),
											)
										}
