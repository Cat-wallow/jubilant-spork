...
export default function InvitationCard({ invitation, onSuccess }: InvitationCardProps) {
  ...
  const getStatusBadge = () => {
    ...
    return (
      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${badges[invitation.status]}`}>
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm transition-shadow hover:shadow-md dark:border-gray-700 dark:bg-navy-800">
      <div className="flex items-start justify-between">
        ...
        <div className="flex-1">
          ...
          <div className="mb-3 flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
            ...
            <span>Diundang oleh: <span className="font-medium text-navy-700 dark:text-white">{invitation.inviter.name}</span></span>
          </div>
          ...
          {invitation.status === 'pending' && !showConfirm && (
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={acceptMutation.isPending || rejectMutation.isPending}
                className="rounded-lg border border-red-500 px-4 py-2 text-sm font-medium text-red-500 transition-colors hover:bg-red-50 dark:hover:bg-red-900/20 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                ✕ Tolak
              </button>
            </div>
            ...
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-800">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={acceptMutation.isPending || rejectMutation.isPending}
                  className="rounded border border-gray-300 px-3 py-1 text-xs font-medium text-gray-700 transition-colors hover:bg-gray-100 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700 disabled:opacity-50"
                >
                  Batal
                </button>
              </div>
