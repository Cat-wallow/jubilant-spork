...
function CheckTable(props: { tableData: any }) {
  ...
  const columns = [
    ...
    columnHelper.accessor('artworks', {
      ...
      cell: (info) => (
        <p className="text-md font-medium text-gray-600 dark:text-white">{info.getValue()}</p>
      ),
    }),
  ...
  return (
    <Card extra={'w-full sm:overflow-auto px-6'}>
      <header className="relative flex items-center justify-between pt-4">
        ...
        <button className="dark:active-bg-card-20 linear rounded-[20px] bg-lightPrimary px-4 py-2 text-base font-medium text-brand-500 transition duration-200 hover:bg-gray-100 active:bg-gray-200 dark:bg-card/5 dark:text-white dark:hover:bg-card/10">
          See all
        </button>
      </header>
